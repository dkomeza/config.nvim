...
local function update_display(input_text)
  ...
  -- Check if input ends with / (creating a directory in current context)
  if input_text:match("/$") then
    dir_path = vim.fn.fnamemodify(full_path:sub(1, -2), ":h")
    filename = vim.fn.fnamemodify(full_path:sub(1, -2), ":t")
    is_new_dir = true
  else
...
local function create_file_with_dirs(filepath)
  ...
  if not parent:exists() then
    ...
    vim.notify(
      "Created directories: " .. parent:absolute(),
      vim.log.levels.INFO
    )
  end
